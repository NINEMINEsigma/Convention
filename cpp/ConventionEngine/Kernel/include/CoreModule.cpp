...
public:
	...
	{
		std::lock_guard<std::shared_mutex> lock(global_mutex);
	...
	_Ret_notnull_ void* MemoryAlloc(size_t size)
	{
		...
		std::lock_guard<std::mutex> alloc_lock(allocation_mutex);
...
{
	std::lock_guard<std::shared_mutex> lock(Raw.global_mutex);
