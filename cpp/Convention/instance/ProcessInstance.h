...
namespace Convention
{
	...
	class instance<ProcessIndicator, true, Allocator, IsUnique> :
		public instance<ProcessIndicator, false, Allocator, IsUnique>
	{
	private:
		using _Mybase = instance<ProcessIndicator, false, Allocator, IsUnique>;
#if defined(_WINDOWS)
	...
	public:
		...
		instance();
		instance(const CharIndicator::tag* commandline, const config_type& config = 0);
		instance(const CharIndicator::tag* executer, const CharIndicator::tag* commandline_args, const config_type& config = 0);
	public:
