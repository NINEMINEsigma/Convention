...
_kernel = DynamicLibrary("ConventionEngineKernel.dll")

_kernel_DoesObjectWithInstanceIDExist = _kernel.SetupFunction(
    "DoesObjectWithInstanceIDExist",
    [ctypes.c_int32],
    ctypes.c_bool
)
_kernel_Internal_CloneSingle = _kernel.SetupFunction(
    "Internal_CloneSingle",
    [ctypes.c_void_p],
    ctypes.c_void_p
)
_kernel_Destroy = _kernel.SetupFunction(
    "Destroy",
    [ctypes.c_void_p, ctypes.c_float],
    ctypes.c_void_p
)
_kernel_DestroyImmediate = _kernel.SetupFunction(
    "DestroyImmediate",
    [ctypes.c_void_p, ctypes.c_bool],
    ctypes.c_void_p
)
_kernel_ToString = _kernel.SetupFunction(
    "ToString",
    [ctypes.c_void_p],
    ctypes.c_char_p
)
...
class CEObject(BaseModel, any_class):
    ...
    def GetInstanceID(self) -> int:
        self.EnsureRunningOnMainThread()
    ...
    def _IsNativeObjectAlive(o:'CEObject') -> bool:
        if o._GetCachedPtr() != 0:
            return True
        return CEObject.DoesObjectWithInstanceIDExist(o.GetInstanceID())
    ...
    def DoesObjectWithInstanceIDExist(instance_id:int) -> bool:
        return _kernel_DoesObjectWithInstanceIDExist(instance_id)

    @staticmethod
    @sealed
    def Instantiate(obj:'CEObject') -> 'CEObject':
        CEObject.CheckNullArgument(obj, "将被实例化的对象为空")
        result = CEObject._Internal_CloneSingle(obj)
        if result is None:
            raise RuntimeError(CEObject._CloneDestroyedMessage)
        return result
    
    @staticmethod
    @sealed
    @overload
    def Destroy(obj:'CEObject') -> None:...
    @sealed
    @staticmethod
    @sealed
    def Destroy(obj:'CEObject', *, t:float=0.0) -> None:
        _kernel_Destroy(obj, t)
    
    @staticmethod
    @sealed
    @overload
    def DestroyImmediate(obj:'CEObject') -> None:...
    @sealed
    @staticmethod
    @sealed
    def DestroyImmediate(obj:'CEObject', *, allowDestroyingAssets:bool=False) -> None:
        _kernel_DestroyImmediate(obj, allowDestroyingAssets)
    
    @staticmethod
    @sealed
    def CheckNullArgument(obj:Any, message:str):
        if obj is None:
            raise ValueError(message)

    @staticmethod
    @sealed
    def _Internal_CloneSingle(obj:'CEObject') -> 'CEObject':
        return _kernel_Internal_CloneSingle(obj)
